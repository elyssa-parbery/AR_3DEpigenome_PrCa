...
do
	mkdir ${OUTDIR}/Pairwise_${TIMEPOINT}_Pooled
	Rscript ${CONFIGDIR}/makePeakMatrix_EC.R --twopass --notrans --rda --var cd --scorecol score ${CONFIGDIR}/names-file_3kb_${TIMEPOINT}_Pooled --cutoff 7 ${OUTDIR}//Pairwise_${TIMEPOINT}_Pooled/peakMatrix_LNCaP_${TIMEPOINT}_DHT_Pooled_SubSam_min7_3Kb.txt
done
